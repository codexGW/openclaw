...
cold-start setup, model warm-up, and embedding-index load on gateways where the
first recall after restart needs a larger end-to-end budget.

When the effective budget expires, Active Memory aborts the blocking memory
sub-agent and classifies the run as timeout/no recall. Timeout boilerplate from
the embedded runner, such as a request timed out message, is treated as
diagnostic text and is not injected as `<active_memory_plugin>` context.
...
| `config.setupGraceTimeoutMs` | `number`                                                                                             | Advanced setup budget added to `config.timeoutMs` for cold-start prompt-build and embedded recall runs; defaults to 0 and is capped at 30000 ms. See [Cold-start grace](#cold-start-grace) for v2026.4.x upgrade guidance |
